...
async function connect(config) {
  ...
  connection.on("error", (error) => {
    console.error(`RabbitMQ booking connection error: ${error.message}`);
  });

  connection.on("close", () => {
    if (!stopping) {
      console.error("RabbitMQ booking connection closed. Reconnecting in 5s...");
      setTimeout(() => startRabbitMQConsumer(config), 5000);
    }
  });
...
export async function startRabbitMQConsumer(config) {
  ...
  try {
    ...
    console.error(`Failed to start RabbitMQ booking consumer: ${error.message}. Retrying in 5s...`);
    setTimeout(() => startRabbitMQConsumer(config), 5000);
  }
}
