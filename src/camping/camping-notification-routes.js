...
import {
  validateCampingFilters,
  validateCampingNotificationRequest
} from "./camping-validation.js";
...
export function createCampingNotificationRouter(notificationService) {
  const router = Router();

  router.get("/", asyncRoute(async (request, response) => {
    const filters = validateCampingFilters(request.query);
